...
PATCH_API_URL="$API_URL/$AUTH_CONFIG_NAME?updateMask=descriptedCredential.serviceAccountCredentials.scope"
...
if [[ "$GET_RESPONSE" == *"\"error\": {"* && "$GET_RESPONSE" == *"\"code\": 404"* ]]; then
 ...
 # Make the POST request
  ...
  echo "No error code 404 found, Making a PATCH request."

  # Make the PATCH request
  curl -X PATCH -H "Authorization: Bearer $ACCESS_TOKEN" -H "Content-Type: application/json" -d "$REQUEST_BODY" "$PATCH_API_URL"
fi
